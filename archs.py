...
class shiftmlp(nn.Module):
    ...
    # # the following commented codes have been added to the function forword()
    # def shift(self, x, dim):
    #     x = F.pad(x, "constant", 0)
    #     x = torch.chunk(x, shift_size, 1)
    #     x = [torch.roll(x_c, shift, dim) for x_s, shift in zip(x, range(-self.pad, self.pad+1))]
    #     x = torch.cat(x, 1)
